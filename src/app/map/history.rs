...
use super::{Bundle, Extents, Map, ProvinceKind, Color};
use super::random_color_pure;
...
impl History {
  ...
  pub(super) fn push(&mut self, step: impl Into<Step>) {
  ...
  pub(super) fn last_step_mut(&mut self) -> Option<&mut Step> {
    self.steps.back_mut()
  }
}
...
impl CalculateRecolorMap {
  pub(super) fn new(bundle: &Bundle) -> Option<Self> {
    ...
    for (&previous_color, province_data) in bundle.map.province_data_map.iter() {
      let color = random_color_pure(&colors_list, &bundle.rng, province_data.kind);
