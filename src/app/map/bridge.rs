...
pub(super) fn load_bundle(location: &Location, config: Rc<Config>, rng: RandomHandle) -> Result<Bundle, Error> {
  let (province_image, definition_table, adjacencies_table) = match location {
    Location::Zip(path) => {
      let mut zip = ZipArchive::new(open_file(path)?)?;
      let province_image = read_province_image(zip.by_name("provinces.bmp")?)?;
    ...
    Location::Dir(path) => {
      let province_image = read_province_image(open_file(path.join("provinces.bmp"))?)?;
...
fn construct_map_data(
  ...
  let mut highest_id = definition_table[0].id;
  ...
  for d in definition_table.iter() {
    ...
    highest_id = highest_id.max(d.id);
  ...
  let id_data = config.preserve_ids.then(|| IdData { highest_id });
...
pub fn save_bundle(location: &Location, bundle: &Bundle) -> Result<(), Error> {
  match location {
    Location::Zip(path) => {
      let (definition_table, adjacencies_table) = deconstruct_map_data(bundle)?;
      ...
      write_province_image(&mut zip, &bundle.map.color_buffer)?;
    ...
    Location::Dir(path) => {
      ...
      write_province_image(file, &bundle.map.color_buffer)?;
...
fn deconstruct_map_data(bundle: &Bundle) -> Result<(Vec<Definition>, Vec<Adjacency>), Error> {
...
fn deconstruct_map_data_preserve_ids(_bundle: &Bundle) -> Result<(Vec<Definition>, Vec<Adjacency>), Error> {
  todo!("the preserve_id feature is not complete yet")
}

fn deconstruct_map_data_no_preserve_ids(bundle: &Bundle) -> Result<(Vec<Definition>, Vec<Adjacency>), Error> {
  ...
  Ok((definitions_table, adjacencies_table))
}

fn read_province_image<R: Read>(reader: R) -> Result<RgbImage, Error> {
...
fn write_province_image<W: Write>(mut writer: W, province_image: &RgbImage) -> Result<(), Error> {
