...
fn open_file(path: impl Into<PathBuf>) -> Result<File, ContextualError<io::Error>> {
  OpenOptions::new()
    ...
    .open(path).context("Unable to open file")
}

fn open_file_maybe_not_found(path: impl Into<PathBuf>) -> Result<Option<File>, ContextualError<io::Error>> {
  use std::io::ErrorKind;
  match open_file(path) {
    Ok(value) => Ok(Some(value)),
    Err(err) if err.error.kind() == ErrorKind::NotFound => Ok(None),
    Err(err) => Err(err)
  }
}
...
fn create_file(path: impl Into<PathBuf>) -> Result<File, ContextualError<io::Error>> {
  OpenOptions::new()
    ...
    .open(path).context("Unable to create file")
}

fn create_or_edit_zip<F>(path: impl Into<PathBuf>, callback: F) -> Result<(), Error>
where F: FnOnce(&mut ZipFilesMap) -> Result<(), Error> {
  let path = path.into();
  let comment = format!("Generated by {}", crate::APPNAME);

  match open_file_maybe_not_found(&path)? {
    Some(file) => ZipFilesMap::edit(file, comment, callback)?,
    None => {
      let file = create_file(path)?;
      let mut zip_files_map = ZipFilesMap::new();
      callback(&mut zip_files_map)?;
      zip_files_map.to_writer(file, comment)?;
    }
  };
...
pub(super) fn load_bundle(location: &Location, config: Config) -> Result<Bundle, Error> {
  let (province_image, definition_table, adjacencies_table, rivers) = match location {
    Location::Zip(path) => {
      let zip_files_map = ZipFilesMap::from_reader(open_file(path)?)?;
      let province_image = read_rgb_bmp_image(zip_files_map.try_get("provinces.bmp")?.as_slice())?;
      let definition_table = read_definition_table(zip_files_map.try_get("definition.csv")?.as_slice())?;
      let adjacencies_table = zip_files_map.get("adjacencies.csv").map(Vec::as_slice)
        .map_or_else(|| Ok(Vec::new()), read_adjacencies_table)?;
      let rivers = zip_files_map.get("rivers.bmp").map(Vec::as_slice)
        .map(read_rgb_bmp_image).transpose()?;
      (province_image, definition_table, adjacencies_table, rivers)
    },
    Location::Dir(path) => {
      ...
      let adjacencies_table = open_file_maybe_not_found(path.join("adjacencies.csv"))?
        .map_or_else(|| Ok(Vec::new()), read_adjacencies_table)?;
      let rivers = open_file_maybe_not_found(path.join("rivers.bmp"))?
        .map(read_rgb_bmp_image).transpose()?;
      (province_image, definition_table, adjacencies_table, rivers)
    }
  };
...
pub fn save_bundle(location: &Location, bundle: &Bundle) -> Result<(), Error> {
  ...
  match location {
    Location::Zip(path) => {
      create_or_edit_zip(path, |zip_files_map| {
        write_rgb_bmp_image(zip_files_map.get_insert_new("provinces.bmp"), &bundle.map.base.color_buffer)?;
        write_definition_table(zip_files_map.get_insert_new("definition.csv"), definition_table)?;

        if !adjacencies_table.is_empty() {
          write_adjacencies_table(zip_files_map.get_insert_new("adjacencies.csv"), adjacencies_table)?;
        } else {
          zip_files_map.remove("adjacencies.csv");
        };

        if let Some(id_changes) = id_changes {
          write_id_changes(zip_files_map.get_insert_new("id_changes.txt"), id_changes)?;
        };
