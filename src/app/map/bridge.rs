...
fn open_file(path: impl Into<PathBuf>) -> Result<File, ContextualError<io::Error>> {
  File::open(path).context("Unable to open file")
}

fn create_file(path: impl Into<PathBuf>) -> Result<File, ContextualError<io::Error>> {
  File::create(path).context("Unable to create file")
}

fn create_open_file(path: impl Into<PathBuf>) -> Result<File, ContextualError<io::Error>> {
  OpenOptions::new()
    .read(true).write(true)
    .create(true).truncate(false)
    .open(path).context("Unable to create or open file")
}
...
pub fn save_bundle(location: &Location, bundle: &Bundle) -> Result<(), Error> {
  match location {
    Location::Zip(path) => {
      ...
      let mut zip = ZipWriter::new(create_open_file(path)?);
      zip.set_comment(format!("Generated by {}", crate::APPNAME));

      zip.start_file("provinces.bmp", Default::default())?;
      write_rgb_bmp_image(&mut zip, &bundle.map.base.color_buffer)?;
    ...
    Location::Dir(path) => {
      ...
      write_rgb_bmp_image(file, &bundle.map.base.color_buffer)?;
